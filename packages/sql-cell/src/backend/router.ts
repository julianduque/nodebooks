...
// Helper function to check notebook access (similar to ensureNotebookAccess in backend)
...
const ensureNotebookAccess = async (
...
export const registerSqlRoutes = (
  ...
  app.post("/notebooks/:id/sql", async (request, reply): Promise<void> => {
    ...
    if (!accessRole && requestUser?.role !== "admin") {
      return;
    }
    ...
    const connection: SqlConnection | undefined = sqlConfig.connections.find(
      (candidate: SqlConnection) => candidate.id === payload.data.connectionId
    );
    if (!connection) {
