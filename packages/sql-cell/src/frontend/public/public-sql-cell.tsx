...
import {
  ...
  describeSqlDriver,
  formatSqlTimestamp,
} from "@nodebooks/client-ui/lib/utils";
import type { PublicCellComponentProps } from "@nodebooks/cell-plugin-api";
import type { SqlCell, SqlConnection } from "../../schema.js";

// Simple code block renderer for SQL queries in public view
const renderSqlCodeBlock = (sql: string): string => {
  const escaped = sql
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
  return `<pre><code class="language-sql">${escaped}</code></pre>`;
};

type PublicSqlCellProps = PublicCellComponentProps & {
  cell: SqlCell;
  connections: SqlConnection[];
  theme: "light" | "dark";
};

const PublicSqlCell = ({ cell, connections, theme }: PublicSqlCellProps) => {
  const connection = useMemo(() => {
    ...
    return (
      connections.find(
        (candidate: SqlConnection) => candidate.id === cell.connectionId
      ) ?? null
    );
  ...
  const queryHtml = useMemo(() => {
    ...
    return renderSqlCodeBlock(querySource);
  }, [querySource]);
