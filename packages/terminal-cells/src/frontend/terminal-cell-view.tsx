...
import type { TerminalCell as TerminalCellData } from "../schema.js";
...
type TerminalCellType = TerminalCellData & NotebookCell;

type TerminalCellViewProps = CellComponentProps & {
...
export const TerminalCellView = ({
  ...
  const scopedTerminalId = useMemo(
    () => `terminal-scope-${cell.id}`,
    [cell.id]
  );
  ...
  const topLeftStatus = useMemo(() => {
    ...
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-muted/40 px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border bg-card text-card-foreground shadow-sm">
      ...
      <div className="px-4 pb-4">
        <div
          className="rounded-md border border-border/70 bg-[#050505] mt-6"
          data-terminal-scope={scopedTerminalId}
        >
          <div
            ref={containerRef}
            className={cn(
              "min-h-[260px] w-full font-mono text-[#e5e5e5]",
        ...
        <style>
          {`
            [data-terminal-scope="${scopedTerminalId}"] .xterm {
              padding: 0.45rem 0.65rem;
              box-sizing: border-box;
              border-radius: 8px;
              background-color: #050505;
            }
            [data-terminal-scope="${scopedTerminalId}"] .xterm-viewport,
            [data-terminal-scope="${scopedTerminalId}"] .xterm-screen {
              background-color: #050505;
              border-radius: 6px;
            }
          `}
        ...
        {error ? (
          <p className="mt-3 text-[12px] text-[color:var(--destructive)]">
            {error}
          </p>
        ) : null}
      </div>
